...
export default function RejectApplication({ application_id, onLoad }: { application_id: string; onLoad?: () => void }) {
    ...
    const onSubmit = async (data: FormValues) => {
        try {
            ...
            await axiosClient.post("/manager/applications/message/reject", {
                application_id,
                reason: data.reason,
                file: uploadedFileKey,
            });

            showToast("Ariza muvaffaqiyatli rad etildi", ToastType.Success);
            reset();
            setFiles([]);
            setOpen(false);
            if (onLoad) onLoad();
        ...
        <AlertDialog open={open} onOpenChange={setOpen}>
            <AlertDialogTrigger asChild>
                <Button variant="destructive" size="sm">
                    Arizani rad etish <X className="ml-1 size-4" />
                </Button>
            </AlertDialogTrigger>
            ...
            <AlertDialogContent className="w-[95vw] max-w-lg sm:max-w-xl max-h-[80vh] overflow-y-auto">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    <AlertDialogHeader>
                        <AlertDialogTitle>Arizani rad etish</AlertDialogTitle>
                        <AlertDialogDescription>
                            Iltimos, rad etish sababini kiriting. PDF hujjat biriktirish ixtiyoriy.
                        </AlertDialogDescription>
                    </AlertDialogHeader>

                    <div className="space-y-2">
                        <Textarea placeholder="Rad etish sababi..." {...register("reason")} />
                    ...
                    <AlertDialogFooter className="gap-2">
                        ...
                        <Button type="submit" variant="destructive" disabled={isSubmitting}>
                            {isSubmitting ? "Yuklanmoqda..." : "Rad etish"}
                        </Button>
                    </AlertDialogFooter>
                </form>
            </AlertDialogContent>
