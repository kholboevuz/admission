...
export async function POST(req: NextRequest) {
    try {
        ...
        const file = body?.file ? String(body.file).trim() : null;
        ...
        if (!reason || reason.length < 10) {
            return NextResponse.json({ success: false, error: "Sabab kamida 10 ta belgidan iborat bo‘lishi kerak" }, { status: 400 });
        }

        await connectDB();

        await ModeratorApplicationsModel.findOneAndUpdate(
            { application_id, moderator_pinfl: auth.pinfl },
            {
                $setOnInsert: { application_id, moderator_pinfl: auth.pinfl },
                $push: {
                    comment: {
                        comment: reason,
                        files: file ? [file] : [],
        ...
        await ApplicationsModel.findOneAndUpdate({ _id: application_id }, { application_status: "rejected" });
